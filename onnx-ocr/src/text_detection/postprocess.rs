...
impl PostProcessor {
    ...
    fn box_score_fast(&self, pred: &Array2<f32>, points: &[Point<u32>; 4]) -> f32 {
        ...
        let all_ps: Vec<Point<i32>> = points
            ...
            .map(|p| {
                Point::new(
                    (p.x.to_owned() - min_x as u32) as i32,
                    (p.y.to_owned() - min_y as u32) as i32,
                )
            })
            .collect();
    ...
    pub fn unclip(&self, points: &[Point<u32>; 4], area: f32, arc: f32) -> Vec<Point<u32>> {
        ...
        for i in 0..path.len() {
            ...
            //  let point = Point::new(p.x as u32, p.y as u32);
            unclip_points.push(p);
        }
        //let minx = unclip_points.iter().map(|p| p.x).min().unwrap_or(0);
        //let miny = unclip_points.iter().map(|p| p.y).min().unwrap_or(0);
        let mut res = Vec::new();
        for p in unclip_points {
            let x = p.x.max(0);
            let y = p.y.max(0);
            let np = Point::new(x as u32, y as u32);
            res.push(np);
        }
        res
    }
